...
Each prompt iteration renders a neat visual dashboard header dynamically tracking:
- Elapsed session time
- Token usage and context window percentages
- Active model and agent mode
- Collapsible inline markdown rendering for clear code formatting

The interface is completely native to your terminal, meaning **traditional visual text highlighting** works out of the box. You can seamlessly drag your mouse over transcript logs to select and copy text exactly as you would in standard OS terminals.
...
### Clipboard & Navigation Hotkeys
Instead of typing or losing focus from the input box, you can leverage native hotkeys bound to your host clipboard and transcript history:
- **`Ctrl+K`**: Copies selected text (if you have highlighted it using your mouse) OR instantly copies the last response generated by the AI assistant.
- **`Ctrl+B`**: Explicitly copies ONLY the last response generated by the AI assistant to the system clipboard.
- **`Ctrl+Y`**: Copies the ENTIRE transcript buffer to your clipboard.
- **`Ctrl+G`**: Instantly reads your system clipboard and triggers a query with the clipboard contents as your prompt.
- **`Shift + Up / Down`**: Scroll the transcript log up or down by a single line.
- **`PageUp / PageDown`**: Scroll the transcript log by a half-page.
- **`Home / End`**: Jump to the very top or very bottom of the transcript.
...
### Listing, Installing, and Removing Skills
...
- `/skills remove <name> [--global]`: Removes an installed skill from the local workspace or global configuration.
